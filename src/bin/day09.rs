...
impl FromStr for Move {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(caps) = RE.captures(s) {
            Ok(Move {
                dir: Dir::from_str(caps.get(1).unwrap().as_str()).unwrap(),
...
fn do_moves(input: &Vec<Move>, depth: usize) -> usize {
    ...
    for mv in input {
        for _ in 0..mv.steps {
            ...
            for i in 1..depth {
                let prev = chain.get(i-1).unwrap().clone();
                let cur = chain.get_mut(i).unwrap();
                cur.move_toward(&prev);
            }
            visited.insert(chain[depth-1].hash());
        }
    }
    visited.len()
}

fn part1(input: &Vec<Move>) {
    println!("Part 1: {}", do_moves(input, 2));
}

fn part2(input: &Vec<Move>) {
    println!("Part 2: {}", do_moves(input, 10));
}
